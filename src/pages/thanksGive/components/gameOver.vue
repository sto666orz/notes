<template>
  <div class="popup-bg" :class="{ complete: showPoster }">
    <div v-if="!showPoster" class="popup-content" ref="poster" key="poster">
      ...
      <div class="game-info">
        <p class="info-first">
          <span>游戏时长</span>{{ scoreTime | timeText }}
        </p>
        <p class="info-second"><span>音符Miss</span>{{ missCount }}个</p>
        <p class="info-three">
          <span>最优排名</span>{{ playerRank | rankText }}
        </p>
      </div>
...
export default {
  ...
  methods: {
    ...
    createQrcode() {
      ...
      $http
        .shortUrl({ url: `${window.location.origin}/thanksGive/home${config}` })
        .then((res) => {
          const {
            data: { data, code },
            msg,
          ...
          if (code == 200) {
            ...
            this.qrcodeImage = qrcode._oDrawing._elCanvas.toDataURL(
              "image/png"
            );
